%AIM- Filter ECG Signal with Baseline Wandering (high Frequency Noise)
%      Using high pass butterworth filter with order 2 and cutoff freuency 
%      0.5 to 5 Hz. Analyse changes on ECG
clc;
clear all;
close all;
...
fc1 = 0.5;
fs1 = 1000;
[b11,a11] = butter(2,fc1/(fs1/2),'high');
figure(2)
freqz(b11,a11);
axis tight
title('Butterworth High pass')

c1=filter(b11,a11,ecg)
figure(1)
subplot(2,3,4)
plot(c1)
axis tight
title('Filter Signal by high pass Freq=0.5 Hz')

%Cut-off Frequency=2Hz
fc2 = 2;
fs2 = 1000;
[b12,a12] = butter(2,fc2/(fs2/2),'high');
figure(3)
freqz(b12,a12);
axis tight
title('Butterworth High pass')

c2=filter(b12,a12,ecg)
figure(1)
subplot(2,3,5)
plot(c2)
axis tight
title('Filter Signal by high pass Freq=2 Hz')

%Cut-off Frequency=5Hz
fc3 = 5;
fs3 = 1000;
[b13,a13] = butter(2,fc3/(fs3/2),'high');
figure(4)
freqz(b13,a13);
axis tight
title('Butterworth high pass ')

c3=filter(b13,a13,ecg)
figure(1)
subplot(2,3,6)
plot(c3)
axis tight
title('Filter Signal by high pass Freq=5 Hz ')